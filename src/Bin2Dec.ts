export default class Bin2Dec {
    ...
    binConversor(){
        ...
        for(var charIndex = 0; charIndex <= _bin.length; charIndex++){
            ...
            if(testedChar != "1" && testedChar != "0" && testedChar != ''){this.dontBinaryElements.push(testedChar)}
            (this.dontBinaryElements.length >= 1)?this.permission=false:this.permission=true;
        }
        if(this.permission == true && _bin.length == 8) {
            for(this.positionOfChar = _bin.length-1 ; this.positionOfChar >=0 ; this.positionOfChar--) {
                this.ResultedDecimal += Number(_bin[this.positionOfChar]) 
        ...
        return `[ERROR] Or the error was the number of values are must be inside on 8 digits base and/or
        have don't binary elements
        ? TIP ? -> Must be 8 digits
        ? TIP ? -> The values must be only binaries`;
